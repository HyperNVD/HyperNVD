...
class GradientLoss():
    ...
    def __call__(self,
        ...
        out_x1yt = model(x1yt_current, emb='saved')
        out_xy1t = model(xy1t_current, emb='saved')
...
class FlowAlphaLoss():
    ...
    def __call__(self,
        ...
        output_forward = model(xyt_forward_match.to(device), emb='saved')
        ...
        output_backward = model(xyt_backward_match.to(device), emb='saved')
...
class FlowMappingLoss():
    ...
    def __call__(self,
        ...
        # forward
        xyt_forward_match, indices_forward = get_flow_match(of, of_mask, jif_current, self.rez, self.num_of_frames, True)
        output_forward_match = model(xyt_forward_match.to(device), emb='saved')
        uv_forward_match = output_forward_match['uv']

        # backward
        xyt_backward_match, indices_backward = get_flow_match(of_rev, of_rev_mask, jif_current, self.rez, self.num_of_frames, False)
        output_backward_match = model(xyt_backward_match.to(device), emb='saved')
...
class ResidualConsistentLoss():
    ...
    def __call__(self, samples_batch, 
        ...
        # get uv
        output = model(xyt, emb='saved')
        uv, residual = output['uv'][layer], output['residual'][layer]

        output_patch = model(xyt_patch, emb='saved')
        ...
        params = model.params
        residual_c = getattr(model.net, f'layer_{layer}').residual(uv.detach(), tn, params=get_subdict(params,f'layer_{layer}'))
        residual_c = residual_c.reshape(samples_frame, samples_batch, 1, 3)
        
        residual_c_patch = getattr(model.net, f'layer_{layer}').residual(uv_patch.detach(), tn_patch, params=get_subdict(params,f'layer_{layer}'))
        ...
        output_test = model(xyt_test, emb='saved')
